...
const DATA_PATH: &'static str = "./msm_bases_bn382";
...
fn variable_msm_affine(c: &mut Criterion) {
    ...
    let mut group = c.benchmark_group("variable_base_msm_affine-bn382-variable number of bases = number of scalars");
...
fn variable_msm_fast(c: &mut Criterion) {
    let mut group = c.benchmark_group("variable_base_msm_fast-bn382-variable number of bases = number of scalars");
    let samples = (14..=23).map(|i| 2usize.pow(i)).collect::<Vec<_>>();
