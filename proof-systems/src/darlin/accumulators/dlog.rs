...
use poly_commit::{
    ...
    rng::{FiatShamirRng, FiatShamirRngSeed},
...
impl<G: AffineCurve> CanonicalDeserialize for DLogItem<G> {
    ...
    fn deserialize_uncompressed_unchecked<R: Read>(
        ...
        // GFinal will always be 1 segment and without any shift
        let g_final = Commitment {
            comm: vec![CanonicalDeserialize::deserialize_uncompressed_unchecked(
                &mut reader,
            )?],
            shifted_comm: None,
        };
...
impl<G: AffineCurve, D: Digest> DLogItemAccumulator<G, D> {
    ...
    pub fn succinct_verify_accumulated_items(
        ...
        let fs_rng_init_seed = {
            let mut seed_builder = <<InnerProductArgPC<G, D> as PolynomialCommitment<
                G::ScalarField,
            >>::RandomOracle as FiatShamirRng>::Seed::new();
        ...
        let comms_values = previous_accumulators
            ...
            .map(|(i, acc)| {
                ...
                let labeled_comm = {
                    ...
                    LabeledCommitment::new(format!("check_poly_{}", i), comm, None)
                };
...
impl<G: AffineCurve, D: Digest> ItemAccumulator for DLogItemAccumulator<G, D> {
    ...
    fn check_items<R: RngCore>(
        ...
        let final_comm_keys = accumulators
            .iter()
            .flat_map(|acc| acc.g_final.comm.clone())
            .collect::<Vec<_>>();
    ...
    fn accumulate_items(
        ...
        let opening_proof =
            InnerProductArgPC::<G, D>::open_check_polys(&ck, xi_s.iter(), z, &mut fs_rng).map_err(
                |e| {
                    end_timer!(poly_time);
...
mod test {
    use super::*;
    use poly_commit::{
        ipa_pc::{BatchProof, UniversalParams},
        Evaluations, LabeledPolynomial, PolynomialCommitment, QuerySet,
    };
    ...
    fn get_test_fs_rng<G: AffineCurve, D: Digest>(
    ) -> <InnerProductArgPC<G, D> as PolynomialCommitment<G::ScalarField>>::RandomOracle {
        ...
        <InnerProductArgPC<G, D> as PolynomialCommitment<G::ScalarField>>::RandomOracle::from_seed(
            fs_rng_seed,
        )
    }
        ...
        D: Digest,
    {
        ...
        let pp = if pp.is_some() {
            ...
            InnerProductArgPC::<G, D>::setup(max_degree)?
        };
        ...
        let (ck, vk) = InnerProductArgPC::<G, D>::trim(&pp, supported_degree)?;
        println!("Trimmed");
