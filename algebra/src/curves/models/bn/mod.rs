    ...
    fn final_exponentiation(f: &Self::Fqk) -> Result<Self::Fqk, Error> {
        ...
        match f.inverse() {
            Some(mut f2) => {
                ...
                let y1 = Fp12ParamsWrapper::<P::Fp12Params>::cyclotomic_square(&y0);
                let y2 = Fp12ParamsWrapper::<P::Fp12Params>::cyclotomic_square(&y1);
                let mut y3 = y2 * &y1;
                let y4 = Self::exp_by_neg_x(y3);
                let y5 = Fp12ParamsWrapper::<P::Fp12Params>::cyclotomic_square(&y4);
