...
fn test_fq_add() {
    ...
    .unwrap();
    let f3 = Fq::from_str(
        "277005306594585864802336372223416785563173966008267311744504363842750431482",
    )
    .unwrap();
    assert!(!f1.is_zero());
    assert!(!f2.is_zero());
...
fn test_fq_double_in_place() {
    ...
    .unwrap();
    let f3 = Fq::from_str(
        "1562534956246314889496965500209603570529195334553640330074228285790731594594",
    )
    .unwrap();
    assert!(!f1.is_zero());
    assert!(!f3.is_zero());
...
fn test_fq_double_in_place_thrice() {
    ...
    .unwrap();
    let f3 = Fq::from_str(
        "1619061669249711823627932173542390925308200867699235900803899476011121610431",
    )
    .unwrap();
    assert!(!f1.is_zero());
    assert!(!f3.is_zero());
...
#[test]
fn test_fq_generate_random_tweedle_point() {
    let d = Fq::from_str(
