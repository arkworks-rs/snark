    ...
    CRHGadget: FixedLengthCRHGadget<P::H, ConstraintF>,
{
    ...
    pub fn check_membership_with_index<CS: ConstraintSystem<ConstraintF>>(
        ...
        self.conditionally_check_membership_with_index(cs, parameters, root, leaf, index, &Boolean::Constant(true))
    }
        ...
        CRHGadget: FixedLengthCRHGadget<P::H, ConstraintF>,
{
    pub fn check_update<CS: ConstraintSystem<ConstraintF>>(
        ...
        self.conditionally_check_update(cs, parameters, old_root, new_root, new_leaf, index, &Boolean::Constant(true))
    }
    ...
    pub fn conditionally_check_update<CS: ConstraintSystem<ConstraintF>>(
        ...
        loop {
            ...
            match pair {
                Some((left_hash, right_hash)) => {// Check if the previous_hash matches the correct current hash.
        ...
        ConstraintF: Field,
{
            ...
            T: Borrow<MerkleTreeTwoPaths<P>>,
    {
        ...
        for (i, &(ref l, ref r)) in paths.old_path.path.iter().enumerate() {
            let l_hash =
                HGadget::OutputGadget::alloc(&mut cs.ns(|| format!("old_path_l_child_{}", i)), || {
                    Ok(l.clone())
                })?;
            let r_hash =
                HGadget::OutputGadget::alloc(&mut cs.ns(|| format!("old_path_r_child_{}", i)), || {
                    Ok(r.clone())
                })?;
        ...
        for (i, &(ref l, ref r)) in paths.new_path.path.iter().enumerate() {
            let l_hash =
                HGadget::OutputGadget::alloc(&mut cs.ns(|| format!("new_path_l_child_{}", i)), || {
                    Ok(l.clone())
                })?;
            let r_hash =
                HGadget::OutputGadget::alloc(&mut cs.ns(|| format!("new_path_r_child_{}", i)), || {
                    Ok(r.clone())
                })?;
...
mod test {
    ...
    fn generate_merkle_tree_and_test_update(old_leaves: &HashMap<usize, [u8; 2]>, new_leaves: &HashMap<usize, [u8; 2]>) -> () {
        let mut rng = XorShiftRng::seed_from_u64(9174123u64);
        ...
        for (i, new_leaf) in new_leaves.iter() {
            ...
            assert!(update_proof.verify(&crh_parameters, &old_root, &new_root, &new_leaf, *i).unwrap());
            assert!(new_leaf_membership_proof.verify_with_index(&crh_parameters, &new_root, &new_leaf, *i).unwrap());
            ...
            update_proof_cw.check_update(
                ...
                .unwrap();
            new_leaf_membership_proof_cw.check_membership_with_index(
                &mut cs.ns(|| format!("new_witness_check_with_index_{}", i)),
                &crh_parameters,
                &new_root_gadget,
                &leaf_g,
