...
impl<P: MerkleTreeConfig> MerkleTreePath<P> {
    pub fn verify<L: ToBytes>(
        ...
        if !self.path.is_empty() {
            ...
            // Calculate the last bit of the index, starting with 1, left leaf odd, right leaf even
...
impl<P: MerkleTreeConfig> MerkleTreeTwoPaths<P> {
    pub fn verify<L: ToBytes>(
        ...
        if self.old_path.path.len() != (P::HEIGHT - 1) as usize || self.new_path.path.len() != (P::HEIGHT - 1) as usize {
            return Ok(false);
        }
        // Check that the given leaf matches the leaf in the membership proof.
        let mut buffer = [0u8; 128];
...
impl<P: MerkleTreeConfig> MerkleHashTree<P> {
    ...
    pub fn new<L: ToBytes>(
        ...
        for (i, leaf) in leaves.iter() {
            tree.insert(last_level_index + *i, hash_leaf::<P::H, _>(&parameters, leaf, &mut buffer)?);
        }
        ...
        for level in 0..Self::HEIGHT  {
            ...
            for current_index in &middle_nodes {
                ...
                tree.insert(*current_index, hash_inner_node::<P::H>(
                    &parameters,
                    &left_hash,
                    &right_hash,
                    &mut buffer,
    ...
    pub fn update_and_prove<L: ToBytes>(
        ...
        loop {
            ...
            self.tree.insert(current_node, hash_inner_node::<P::H>(
                &self.parameters,
                &left_hash,
                &right_hash,
                &mut buffer
...
pub(crate) fn gen_empty_hashes<P: MerkleTreeConfig>(parameters: &<P::H as FixedLengthCRH>::Parameters) -> Result<Vec<<P::H as FixedLengthCRH>::Output>, Error> {
    let mut buffer = [0u8; 128];
...
mod test {
    ...
    fn generate_merkle_tree_and_test_membership<L: ToBytes + Clone + Eq>(leaves: &HashMap<usize, L>) -> () {
        ...
        for (i, leaf) in leaves.iter() {
            ...
            assert!(proof.verify_with_index(&crh_parameters, &root, &leaf, *i as usize).unwrap());
        }
    }
    ...
    fn generate_merkle_tree_with_bad_root_and_test_membership<L: ToBytes + Clone + Eq>(leaves: &HashMap<usize, L>) -> () {
    ...
    fn generate_merkle_tree_and_test_update<L: ToBytes + Clone + Eq>(old_leaves: &HashMap<usize, L>, new_leaves: &HashMap<usize, L>) -> () {
        let mut rng = XorShiftRng::seed_from_u64(9174123u64);
        ...
        for (i, new_leaf) in new_leaves.iter() {
            ...
            match old_leaf_option {
                Some(old_leaf) => {
                    ...
                    assert!(old_leaf_membership_proof.verify_with_index(&crh_parameters, &old_root, &old_leaf, *i as usize).unwrap());
                    assert!(!(old_leaf_membership_proof.verify_with_index(&crh_parameters, &new_root, &old_leaf, *i as usize).unwrap()));
                    assert!(new_leaf_membership_proof.verify_with_index(&crh_parameters, &new_root, &new_leaf, *i as usize).unwrap());
                    assert!(!(new_leaf_membership_proof.verify_with_index(&crh_parameters, &new_root, &old_leaf, *i as usize).unwrap()));

                    assert!(update_proof.verify(&crh_parameters, &old_root, &new_root, &new_leaf, *i as usize).unwrap());
                },
                None => {
                    let update_proof = tree.update_and_prove(*i, &new_leaf).unwrap();
                    let new_leaf_membership_proof = tree.generate_proof(*i, &new_leaf).unwrap();
