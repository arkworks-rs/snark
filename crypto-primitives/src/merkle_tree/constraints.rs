...
where
    ...
    ConstraintF: Field,
{
    fn alloc_constant<T, CS: ConstraintSystem<ConstraintF>>(mut cs: CS, val: T) -> Result<Self, SynthesisError>
    where
        T: Borrow<MerkleTreePath<P>>,
    {
        ...
        for (i, &(ref l, ref r)) in val.borrow().path.iter().enumerate() {
            let l_hash =
                HGadget::OutputGadget::alloc_constant(&mut cs.ns(|| format!("l_child_{}", i)), l.clone())?;
            let r_hash =
                HGadget::OutputGadget::alloc_constant(&mut cs.ns(|| format!("r_child_{}", i)), r.clone())?;
